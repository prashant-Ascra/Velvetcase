...
public static MotionEvent lastmotionevent;
// minimap variables
...
public void zoomTo(final float zoom, final float x, final float y) {
    ...
    public void resetZoomView(Boolean flag){
        ZoomFlag = true;
        count = 0;
        resizeFlag = flag;
        try{
...
private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
    ...
    public void applyPinchZoom(){
        count++;
        if(count == 1) {
            scaleXAnimator = ObjectAnimator.ofFloat(this, "scaleX", 1.5f);
            scaleXAnimator.setRepeatMode(ValueAnimator.REVERSE);
            scaleXAnimator.setRepeatCount(1);
            scaleXAnimator.setDuration(300);

            scaleYAnimator = ObjectAnimator.ofFloat(this, "scaleY", 1.5f);
            ...
            set.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    ((MainActivity) context).TransferNextActivtiy();
...
private void processDoubleTouchEvent(final MotionEvent ev) {
    ...
    public void smoothZoomTo(final float zoom1, final float x, final float y) {
        if(ZoomFlag){
            ZoomFlag = false;
            defaultX = x;
            defaultY = y;
        ...
        smoothZoom = clamp(1.0f, zoom1, maxZoom);
        if(smoothZoom >= 1.17f ){

//        zoomTo(1.0f,85.0f,103f);
            smoothZoom = 1.0f;
            maxZoom = 2.0f;
            zoom = 1.0f;
            zoomX = 84.064995f;
...
//    }
    ...
    public void smoothZoomToMinimum() {

        smoothZoom = clamp(1.0f, 1.0f, 1.0f);
